...
iphone.small.matrix %>%
  select(starts_with("ios"), starts_with("iphone")) -> iphonedata

galaxy.small.matrix %>% 
  select(starts_with("google"), starts_with("samsung"), starts_with("galaxy")) -> galaxydata
...
## Near zero variance of iphone
nzv.metrics.iphone <- nearZeroVar(iphoneready, saveMetrics = TRUE)
nzv.iphone <- nearZeroVar(iphoneready, saveMetrics = FALSE) 
iphoneready2 <- iphoneready[,-nzv.iphone]
